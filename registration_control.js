...
function checkUser() {
    ...
    if (username === '') {
        ...
        fetch('check_username.php?q=' + username_encoded).then(OnUserResponse).then(OnUserJson);
    }
}
...
function checkEmail() {
    ...
    if (email === '') {
        ...
        fetch('check_email.php?q=' + email_encoded).then(onEmailResponse).then(onEmailJson);
    }
}
